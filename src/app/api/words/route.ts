...
async function getRawWordlist() {
  ...
  if (token && repo) {
    try {
      ...
      return { content: "", sha: null, error: message };
    }
  }
...
async function saveWordlist(newContent: string, sha: string | null) {
  ...
  if (token && repo) {
    const base64Content = Buffer.from(normalizedContent, "utf8").toString(
      "base64",
    );
    const response = await fetch(
  ...
  try {
    ...
    throw new Error("本地写入降级失败: Unknown error");
  }
}
